...
const runReleaseScript = ({
  ...
  const { projectFolder } = projectInfo;
  const name = `Preparing release of ${projectName}@${newVersion}`;
...
const runPublishScript = ({
  ...
  const { projectFolder } = projectInfo;
  const name = `Publish ${projectName}@${newVersion}`;

  if (dryRun) {
    return new Promise(resolve => {
      console.log(chalk.italic(`Spawning process in ${projectFolder}`));
      if (scriptType === 'yarn') {
        console.log(chalk.italic(`yarn run ${publishScript}`));
      } else {
        console.log(chalk.italic(`npm run ${publishScript}`));
      }
      resolve(null);
    });
  }
